...
| `APP_TYPE_ANDROID` | **Auto-detected by the agent** by scanning `android/app/src/main/AndroidManifest.xml` for non-ReactActivity Activities (see `AGENTS-android-setup.md` Step 1). After detection, ask the developer to **confirm**: *"I detected [standard React Native / hybrid] based on your AndroidManifest.xml. Is that correct?"* Do not ask the developer to classify upfront -- detect first, then confirm. |
...
`AGENTS-android-setup.md` Step 1 determines whether the app is standard React Native or hybrid by inspecting `android/app/src/main/AndroidManifest.xml`. The agent auto-detects the classification, then asks the developer to confirm.
