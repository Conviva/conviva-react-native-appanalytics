...
- **Try-catch import requirement:** Whenever adding a try-catch block to any native file, verify that the exception type used in the catch clause is imported. Rules per language:
  - **Java**: `Exception` is `java.lang.Exception` and is auto-imported in every Java file -- no explicit import statement is required.
  - **Kotlin**: `Exception` is in the `kotlin` package and is auto-imported in every Kotlin file -- no explicit import statement is required.
  - **Objective-C**: `NSException` and `NSLog` are part of the `Foundation` framework. Add `@import Foundation;` at the top of the file if it is not already present. Do not assume Foundation is available transitively -- always verify and add it explicitly.
  - **Swift**: `guard let` nil-checks and `do/try/catch` blocks use Swift built-in types (`Error`) -- no import is required.
  - **JavaScript / TypeScript**: `try/catch` and `Error` are language built-ins -- no import is required.
...
| Babel plugin | **For CONVIVA_RN_VERSION <= 0.2.8:** `./node_modules/@convivainc/conviva-react-native-appanalytics/instrumentation/index.js` and `add-react-displayname` added to `plugins`; `babel-plugin-add-react-displayname` `0.0.5` added to `devDependencies`; `babel-types` `^6.26.0` and `babel-template` `^6.26.0` added to `devDependencies` if absent (legacy Babel 6 packages required by `instrumentation/index.js` -- not installed automatically in Babel 7 projects; skip if already present at any `6.x` version). **For CONVIVA_RN_VERSION >= 0.3.0:** `'@convivainc/conviva-react-native-appanalytics/plugin'` added to `plugins` in `babel.config.js`; no extra dependencies needed -- `@babel/plugin-transform-react-display-name` is a dependency of the Conviva package and resolves from its own `node_modules` automatically; do NOT add it to `plugins` or `devDependencies` |
